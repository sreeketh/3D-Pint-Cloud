...
def draw_box_3d(image, corners, color=(0, 0, 255),label='sree'):
    ''' Draw 3d bounding box in image
    ...
    for ind_f in range(3, -1, -1):
        ...
        for j in range(4):
            ...
            cv2.putText(image,label,(corners[face_idx[0][3], 0], corners[face_idx[0][3], 1]),cv2.FONT_HERSHEY_SIMPLEX,1,color=color,lineType=cv2.LINE_AA)
        if ind_f == 0:
            cv2.line(image, (corners[f[0], 0], corners[f[0], 1]),
                     (corners[f[2], 0], corners[f[2], 1]), color, 1, lineType=cv2.LINE_AA)
            cv2.line(image, (corners[f[1], 0], corners[f[1], 1]),
                     (corners[f[3], 0], corners[f[3], 1]), color, 1, lineType=cv2.LINE_AA)
            cv2.putText(image, label, (corners[face_idx[0][3], 0], corners[face_idx[0][3], 1]),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, color=color, lineType=cv2.LINE_AA)
...
def show_rgb_image_with_boxes(img, labels, calib):
    for box_idx, label in enumerate(labels):
        ...
        img = draw_box_3d(img, corners_2d, color=cnf.colors[int(cls_id)],label=list(cnf.CLASS_NAME_TO_ID.keys())[int(cls_id)])
